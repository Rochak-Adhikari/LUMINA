...
function App() {
    ...
    // Phase D.1.a: Connection status tracking
    const [connectionStatus, setConnectionStatus] = useState('connected'); // 'connected', 'reconnecting', 'offline'
    const [modelStatus, setModelStatus] = useState('disconnected'); // 'connecting', 'connected', 'disconnected', 'reconnecting'
    // Auth State
    ...
    // Phase E2: Memory suggestion state (popup DISABLED by default — lifecycle uses backend auto-store)
    const MEMORY_POPUP_ENABLED = false;
    const [memorySuggestions, setMemorySuggestions] = useState([]); // Array of {temp_id, type, content, confidence, reason}
    const seenSuggestionIds = React.useRef(new Set());

    // Phase T1.5: Lifecycle event dedupe (bounded Set, separate from suggestion dedupe)
    ...
    useEffect(() => {
        ...
        socket.on('status', (data) => {
            ...
            if (data.msg === 'Lumina Started') {
                setStatus('Model Connected');
            } else if (data.msg === 'Lumina Stopped') {
                setStatus('Connected');
            }
        });
        ...
        // Phase E2: Handle memory suggestions (popup disabled by default, dedupe by temp_id)
        socket.on('memory_suggestion', (data) => {
            console.log('[MEMORY SUGGEST] Received (popup disabled):', data.temp_id);
            if (!MEMORY_POPUP_ENABLED) return; // Popup disabled — backend handles lifecycle
            if (seenSuggestionIds.current.has(data.temp_id)) return; // Dedupe
            seenSuggestionIds.current.add(data.temp_id);
            setMemorySuggestions(prev => [...prev, {
        ...
        // Phase T1: memory_lifecycle_event is handled in its own useEffect below (with cleanup)
    ...
    // ========================================
    // Memory lifecycle event listener (debug logging only — no chat bubbles)
    // Lumina speaks through Gemini Live audio; lifecycle events are backend-only signals.
    // ========================================
    useEffect(() => {
        const handleLifecycleEvent = (data) => {
            ...
            if (lifecycleSeenRef.current.size > 300) {
                lifecycleSeenRef.current.clear();
                lifecycleSeenRef.current.add(key);
            }
            console.log('[MEMORY LIFECYCLE]', data.event, data.memory_id ?? data.id, data.type);
        };

        socket.on('memory_lifecycle_event', handleLifecycleEvent);
        return () => {
            socket.off('memory_lifecycle_event', handleLifecycleEvent);
        };
    }, []);
    ...
    // Phase E2: Memory suggestion decision handlers
    const handleMemoryDecision = (temp_id, accept) => {
        console.log(`[MEMORY DECISION] ${accept ? 'Accept' : 'Reject'} suggestion:`, temp_id);
        socket.emit('memory_decision', { temp_id, accept });
        // Remove from local state
        setMemorySuggestions(prev => prev.filter(s => s.temp_id !== temp_id));
    };
        ...
        <div className="h-screen w-screen bg-black text-cyan-100 font-mono overflow-hidden flex flex-col relative selection:bg-cyan-900 selection:text-white">
            ...
            <div className="z-50 flex items-center justify-between p-2 border-b border-cyan-500/20 bg-black/40 backdrop-blur-md select-none sticky top-0" style={{ WebkitAppRegion: 'drag' }}>
                <div className="flex items-center gap-4 pl-2">
                    ...
                    {/* Phase D.1.a: Connection Status Indicator */}
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                {/* Phase E2: Memory Suggestions UI (disabled by default — lifecycle uses voice/text confirmation) */}
                {MEMORY_POPUP_ENABLED && memorySuggestions.length > 0 && (
                    <div className="fixed bottom-24 left-1/2 transform -translate-x-1/2 z-50 max-w-md w-full px-4">
                        {memorySuggestions.map((suggestion) => (
                            <div
                                key={suggestion.temp_id}
                                className="mb-3 backdrop-blur-xl bg-purple-900/30 border border-purple-500/30 rounded-lg p-4 shadow-2xl animate-slide-up"
                            >
                                <div className="flex items-start justify-between mb-2">
                                    <div className="flex-1">
                                        ...
                                        <p className="text-xs text-purple-300/70 mt-1">
                                            💬 Say "yes" or "no" to respond by voice
                                        </p>
                                    </div>
                                </div>
                                <div className="flex gap-2 mt-3">
                                    <button
                                        onClick={() => handleMemoryDecision(suggestion.temp_id, true)}
                                        className="flex-1 bg-green-600/80 hover:bg-green-500 text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all duration-200 hover:scale-105"
                                    >
                                        ✓ Yes, remember this
                                    </button>
                                    <button
                                        onClick={() => handleMemoryDecision(suggestion.temp_id, false)}
                                        className="flex-1 bg-gray-700/80 hover:bg-gray-600 text-white text-sm font-semibold py-2 px-4 rounded-lg transition-all duration-200 hover:scale-105"
                                    >
                                        ✗ No, skip
                                    </button>
                                </div>
