                        ...
                        for fc in response.tool_call.function_calls:
                            ...
                            if fc.name in _ACTION_TOOLS_AUTOCONFIRM or fc.name in _ACTION_TOOL_NAMES:
                                ...
                                if fc.name == "generate_cad":
                                    ...
                                    # Parse params (LLM sends as JSON string)
                                    import json as _json
                                    if isinstance(params_raw, str):
                                        try:
                                            parsed_params = _json.loads(params_raw)
                                    ...
                                    # Build context with current permissions
                                    from tools.browser_control import execute_browser_intent as _exec_browser
                                    _ctx = {"tool_permissions": self.permissions}
                                    ...
                                    if result_dict.get("data"):
                                        # Include key data for the LLM
                                        data = result_dict["data"]
                                        if data.get("url"):
                                            result_str += f"\nURL: {data['url']}"
                                        if data.get("title"):
                                            result_str += f"\nTitle: {data['title']}"
                                        if data.get("dom_text"):
                                            result_str += f"\nPage content:\n{data['dom_text'][:2000]}"
                                ...
                                elif fc.name == "local_browser_control":
                                    action = fc.args.get("action", "")
                                    params_raw = fc.args.get("params", "{}")
                                    _TOOL_TIMEOUT = 30  # seconds — must exceed Brave startup (15s) + goto (20s)
                                    _tool_t0 = time.time()
                                    ...
                                    _ctx = {
                                        ...
                                        "confirmation_mode": self._browser_confirmation_mode,
                                    }
                                    try:
                                        ...
                                        print(f"[TOOL] done name=local_browser_control action={action} ms={_tool_ms}")
                                    except asyncio.TimeoutError:
                                        _tool_ms = int((time.time() - _tool_t0) * 1000)
                                        print(f"[TOOL] timeout name=local_browser_control action={action} ms={_tool_ms}")
                                        result_dict = {"ok": False, "message": f"Browser action '{action}' timed out after {_TOOL_TIMEOUT}s.", "data": {}}

                                    # -- Handle gated action confirmation (e.g. type_text) --
                                    if result_dict.get("needs_confirmation"):
                                        if self.on_tool_confirmation:
                                            ...
                                            self._pending_confirmations[_gate_id] = _gate_future
                                            self.on_tool_confirmation({
                                                "id": _gate_id,
                                                "tool": fc.name,
                                                "args": {**fc.args, "_gated_action": action,
                                                         "_detail": _gate_detail}
                                            })
                                            try:
                                                _confirmed = await _gate_future
                                            finally:
                                                self._pending_confirmations.pop(_gate_id, None)
                                    ...
                                    if result_dict.get("data"):
                                        ...
                                        if data.get("tabs"):
                                            result_str += f"\nTabs: {data['tabs']}"
                                        if data.get("matched"):
                                            result_str += f"\nMatched element: {data['matched']}"
                                        if data.get("closest"):
                                            result_str += f"\nClosest candidates: {data['closest']}"
                                        if data.get("clickables"):
                                            cl = data["clickables"][:20]
                                            result_str += f"\nClickable elements ({len(data['clickables'])} total, showing {len(cl)}):\n"
                                            for i, c in enumerate(cl):
                                                result_str += (f"  [{i}] <{c['tag']}> \"{c.get('text','')[:50]}\" "
                                                               f"aria=\"{c.get('aria_label','')[:30]}\" "
                                                               f"conf={c.get('confidence',0)}\n")
                                        if data.get("focused"):
                                            result_str += f"\nFocused element: {data['focused']}"
                                        if "found" in data:
                                            result_str += f"\nText found: {data['found']}"
                                        if data.get("timed_out"):
                                            result_str += "\nTimed out waiting for text"
                                        if "tab_index" in data:
                                            result_str += f"\nTab index: {data['tab_index']}"
                                        if "active_tab_index" in data:
                                            result_str += (f"\nActive state: tab={data.get('active_tab_index')}"
                                                           f", loading={data.get('is_loading')}"
                                                           f", viewport={data.get('viewport')}")
                                        if data.get("recovery"):
                                            result_str += f"\nRecovery: {data['recovery']}"
                                        if data.get("clicked"):
                                            result_str += (f"\nClicked: text='{data.get('text','')[:50]}' "
                                                           f"tag={data.get('tag','')} score={data.get('score',0)} "
                                                           f"bbox={data.get('bbox',[])}")
                                        if "remaining_tabs" in data:
                                            result_str += f"\nRemaining tabs: {data['remaining_tabs']}"
                                        if data.get("inputs"):
                                            inp = data["inputs"][:10]
                                            result_str += f"\nInputs ({len(data['inputs'])} total):\n"
                                            for i, f_inp in enumerate(inp):
                                                result_str += (f"  [{i}] placeholder='{f_inp.get('placeholder','')[:40]}' "
                                                               f"type={f_inp.get('type','')} bbox={f_inp.get('bbox',[])}\n")
                                ...
                                # ── Phase M: Mark-XXX action tool dispatch ──────────────
