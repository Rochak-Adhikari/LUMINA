"""
actions/desktop_control.py — Lumina Desktop & Wallpaper Management Action

Ported from Mark-XXX-main/actions/desktop.py.
Adaptations:
  - API key via _gemini_helper (Lumina .env pattern)
  - Entry function named `desktop_control` (was `desktop_control` in Mark-XXX too)
  - Log prefix changed to Lumina style
  - Gemini model name updated to gemini-2.5-flash
"""
...
try:
    ...
    pyautogui = None  # type: ignore  # optional — needed only for 'task' action
...
def desktop_control(
    ...
    """
    Lumina desktop & wallpaper management action.
    ...
    parameters:
        ...
        task   : NL description for AI-powered actions
    """
    ...
    try:
        if action == "wallpaper":
            ...
            if code == "UNSAFE":
                result = "I cannot perform that desktop action safely."
