...
def _resolve_path(raw: str) -> Path:
    """Resolves a path from user input, supporting shortcuts."""
    shortcuts = {
...
def file_controller(
    ...
    """
    Lumina file management action.

    parameters:
        action      : list | create_file | create_folder | delete | move | copy |
                      rename | read | write | find | largest | disk_usage |
                      organize_desktop | info
        path        : Directory or file path (supports shortcuts: desktop, downloads, documents, home)
        name        : File/folder name
        content     : Content for write/create
        destination : Target path for move/copy
        new_name    : New name for rename
        extension   : File extension filter for find
        max_results : Max results for find (default 20)
        count       : Max results for largest (default 10)
        append      : bool for write action
    """
