...
def _load_user_profile(session_memory=None) -> dict:
    """
    Load user profile data for form filling.
    Tries session_memory (Lumina MemoryStore) first, then falls back to empty dict.
    """
    if session_memory:
        try:
            facts = session_memory.search_memories("name email city age", limit=10)
            if facts:
                profile = {}
                for mem in facts:
                    ...
                    if "email" in content and "@" in content:
                        import re
                        match = re.search(r'[\w.+-]+@[\w-]+\.[\w.]+', mem["content"])
                        if match:
                            profile.setdefault("email", match.group())
                return profile
        except Exception:
            pass
    return {}
...
def generate_random_data(data_type: str) -> str:
    ...
    if dt == "city":      return random.choice(["New York", "Los Angeles", "Chicago", "Houston", "Phoenix"])
...
def _screenshot(save_path: str = None) -> str:
    _ensure_pyautogui()
    if not save_path:
        save_path = str(Path.home() / "Desktop" / "lumina_screenshot.png")
...
def _analyze_screen_for_element(description: str):
    """Use Gemini to find an element on screen. Returns (x, y) or None."""
    try:
        import io, re
...
def computer_control(
    ...
    """
    Lumina universal computer control action (PyAutoGUI).

    Actions: type, smart_type, click, left_click, double_click, right_click,
             move, drag, hotkey, press, scroll, copy, paste, screenshot,
             wait, wait_image, clear_field, focus_window, screen_size,
             screen_find, screen_click, random_data, user_data
    """
    action = (parameters or {}).get("action", "").lower().strip()

    if not action:
        return "Please specify an action for computer_control."

    if player:
        player.write_log(f"[Computer] {action}")

    print(f"[ComputerControl] ▶️ Action: {action}")
    ...
    try:
        if action == "type":
            ...
            return _click(
                ...
                button="left", clicks=1, image=parameters.get("image")
            )

        elif action == "double_click":
            return _click(
                x=parameters.get("x"), y=parameters.get("y"),
                button="left", clicks=2, image=parameters.get("image")
            )
            ...
            return _drag(
                x1=int(parameters.get("x1", 0)), y1=int(parameters.get("y1", 0)),
                x2=int(parameters.get("x2", 0)), y2=int(parameters.get("y2", 0))
            )
            ...
            profile = _load_user_profile(session_memory)
