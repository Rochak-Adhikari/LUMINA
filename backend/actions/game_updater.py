...
# ── Standard Platform checks (replacing custom config imports) ──────────────────
def get_os() -> str:
...
def _click_first_profile_by_screenshot() -> bool:
    try:
        ...
        if not colorful.any():
            print("[GameUpdater] ⚠️ Profile avatar color not detected — guessing click")
        ...
        print(f"[GameUpdater] 🎯 Profile avatar detected at ({abs_x}, {abs_y}) — clicking")
        pyautogui.click(abs_x, abs_y)
        return True

    except ImportError as e:
        print(f"[GameUpdater] ⚠️ Missing library for screen click: {e}")
        return False
...
def _handle_steam_profile_selection() -> bool:
    ...
    try:
        ...
        if not is_small and white_pixels <= 100:
            print("[GameUpdater] ℹ️ Profile dialog not present — Steam is already logged in")
            return False
    except ImportError:
        pass
    except Exception:
...
def _ensure_steam_running(steam_path: Path) -> bool:
    ...
    if not exe.exists():
        print(f"[GameUpdater] ❌ Steam not found at: {exe}")
        return False
...
def _search_steam_appid(game_name: str) -> tuple[str | None, str | None]:
    ...
    if name_lower in _KNOWN_APPIDS:
        ...
        print(f"[GameUpdater] 📖 Known App: {canonical} ({app_id})")
    ...
    try:
        ...
        if items:
            best = items[0]
            print(f"[GameUpdater] 🌐 Store API Match: {best['name']} ({best['id']})")
            return str(best["id"]), best["name"]
    except Exception as e:
        print(f"[GameUpdater] ⚠️ AppID store search failed: {e}")
...
def _watch_and_shutdown(steam_path: Path, speak=None,
    ...
    print("[GameUpdater] Shutdown watcher active...")
    ...
    for _ in range(24):
        ...
        if active:
            ...
            if speak:
                speak(f"Download started for {names}. I will shut down when complete.")
            break
    else:
        return
    ...
    if speak:
        speak("Download took too long. Auto-shutdown cancelled.")
...
def _schedule_mac(hour: int, minute: int) -> str:
    ...
    plist_path  = plist_dir / "com.lumina.gameupdater.plist"
    script_path = Path(__file__).resolve()
    plist_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN"
  "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0"><dict>
    <key>Label</key><string>com.lumina.gameupdater</string>
...
def _cancel_scheduled_update() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Delete", "/TN", "Lumina_GameUpdater", "/F"],
            capture_output=True, text=True
        )
        return ("Scheduled update cancelled."
                if result.returncode == 0 else "No scheduled update found.")
    if is_mac():
        plist_path = Path.home() / "Library" / "LaunchAgents" / "com.lumina.gameupdater.plist"
    ...
    try:
        existing = subprocess.run(["crontab", "-l"], capture_output=True, text=True)
        lines    = [l for l in existing.stdout.splitlines()
                    if "Lumina_GameUpdater" not in l]
...
def _get_schedule_status() -> str:
    if is_windows():
        result = subprocess.run(
            ["schtasks", "/Query", "/TN", "Lumina_GameUpdater", "/FO", "LIST"],
            capture_output=True, text=True
        )
        if result.returncode != 0:
...
def game_updater(
    ...
    """
    Control game updates and installations for Steam and Epic Games.

    parameters:
        action             : update | install | list | download_status | schedule | cancel_schedule | schedule_status (default: update)
        platform           : steam | epic | both (default: both)
        game_name          : Game name (optional)
        app_id             : Game AppID for Steam (optional)
        hour               : Hour for daily update schedule (0-23, default: 3)
        minute             : Minute for daily update schedule (0-59, default: 0)
        shutdown_when_done : Auto-shutdown computer when downloads finish (true/false, default: false)
    """
    p         = parameters or {}
                    ...
                    if not is_installed:
                        ...
                        if shutdown:
                            threading.Thread(
                                ...
                                kwargs={"steam_path": steam_path, "speak": None},
                                daemon=True
                            ).start()
