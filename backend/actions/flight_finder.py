...
def _search_flights_browser(
    ...
    from actions import run_coroutine
    ...
    async def _async_search():
        ...
        conn = await ctrl.ensure_connected("open_url")
        if not conn["success"]:
            raise RuntimeError(f"CDP connection failed: {conn.get('error')}")

        await ctrl._ensure_lumina_tab()
        nav = await ctrl.open_url(url)
        if not nav["success"]:
            raise RuntimeError(f"Navigation failed: {nav.get('error')}")

        # Wait for flight listings to render
        await asyncio.sleep(5)

        # Retrieve main page body text
        text = await ctrl._page.inner_text("body")
        return text or ""

    try:
        raw = run_coroutine(_async_search())
        return raw, url
    except Exception as e:
        print(f"[FlightFinder] ❌ Browser search failed: {e}")
        return "", url
...
def flight_finder(
    ...
    """
    Find flights between two locations using Google Flights.

    parameters:
        origin      : Departure airport or city (required)
        destination : Arrival airport or city (required)
        date        : Departure date (required)
        return_date : Return date (optional)
        passengers  : Number of passengers (default: 1)
        cabin       : economy | premium | business | first (default: economy)
        save        : Save results to Desktop (optional, default: false)
    """
    ...
    if player:
        player.write_log(f"[FlightFinder] Route: {origin} → {destination} on {date}")

    print(
        f"[FlightFinder] Route: {origin} → {destination} | {date}"
        f"{' → ' + return_date if return_date else ''}"
        f" | {cabin} | {passengers} passengers"
    )
