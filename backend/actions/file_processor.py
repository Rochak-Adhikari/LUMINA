"""
file_processor.py — Lumina Universal File Processor

Ported from Mark-XLVI/actions/file_processor.py, adapted to Lumina's patterns:
  - Uses _gemini_helper for API key/client (not config/api_keys.json)
  - Follows Lumina action signature: file_processor(parameters, response, player, session_memory) -> str
  - Registered via ACTION_REGISTRY in actions/__init__.py

Supported types:
  image   → describe, ocr, resize, convert, compress, crop, info
  pdf     → summarize, extract_text, extract_pages, to_word, info
  ...
  xlsx    → analyze, filter, convert, stats, info
  json    → validate, format, extract, convert, analyze
  code    → explain, review, fix, run, document, info
  audio   → transcribe, trim, convert, info
  video   → trim, extract_audio, extract_frame, info, compress, transcribe, convert
  zip     → list, extract
...
# ── Lumina Gemini client (replaces Mark-XLVI's _get_api_key / _gemini_client) ──

def _gemini_client():
    """Returns a wrapper around Lumina's Gemini genai Client."""
...
def _process_image(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    # Fallback: treat unknown action as AI instruction
...
def file_processor(
    ...
    """
    Lumina file processor action — AI-powered analysis of any uploaded file.

    parameters:
        file_path   : Path to the file to process (required)
        action      : What to do with the file (auto-detected if omitted)
        instruction : Custom instruction for AI processing
    """
    file_path_str = (parameters or {}).get("file_path", "").strip()
    ...
    action      = ((parameters or {}).get("action") or "").lower().strip()
    instruction = (parameters or {}).get("instruction", "")
    params      = {**parameters, "instruction": instruction}

    log_msg = f"[FileProcessor] {file_type.upper()} | {path.name} | action={action or 'auto'}"
    print(log_msg)

    # Unknown file type → attempt AI analysis of raw content
    ...
    try:
        result = handler(path, action, params, None)
