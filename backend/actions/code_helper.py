"""
code_helper.py — Code writing, editing, explaining, running, and debugging.

Imported from Jarvis-MK37 and adapted for Lumina's action system.
Uses Gemini generative API for code generation and analysis.

Signature: code_helper(parameters, response=None, player=None, session_memory=None) -> str
"""
...
def _get_api_key() -> str:
    ...
    if key:
        return key
    # Fallback: read from settings.json in backend dir
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    return DESKTOP / f"lumina_code{ext}"
...
def _take_screenshot() -> Path | None:
    try:
        import pyautogui
        screenshot_path = Path.home() / "Desktop" / f"lumina_debug_{int(time.time())}.png"
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    screen_kw = ["screen", "what's wrong", "screenshot", "this error",
                 "why am i getting", "what error"]
...
def _write(description: str, language: str, output_path: str, player=None) -> tuple:
...
def code_helper(
    ...
    """
    Code writing, editing, explaining, running, building, and debugging.
