...
# ── Safety: blocked destructive patterns ─────────────────────────────────────
BLOCKED_PATTERNS = [
...
def _ask_gemini(task: str) -> str:
    """Use Gemini to convert a natural language task to a CMD command."""
    try:
...
def cmd_control(
    ...
    """
    Lumina CMD / Shell control action.

    parameters:
        task    : Natural language description of what to do (Gemini generates command)
        command : Direct shell command to run (skips Gemini if provided)
        visible : If True, opens in a new visible terminal window (default: True)
    """
    ...
    if not task and not command:
        return "Please describe what you want to do, or provide a command directly."
    ...
    if not command:
        ...
        if command:
            ...
            if command == "UNSAFE":
                return "I cannot generate a safe command for that request."
