...
def brightness_up():
    if _OS == "Windows": pyautogui.hotkey("win", "a"); time.sleep(0.3)
    elif _OS == "Darwin": subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 144'])
    else: subprocess.run(["brightnessctl", "set", "+10%"])


def brightness_down():
    if _OS == "Windows": pyautogui.hotkey("win", "a"); time.sleep(0.3)
    elif _OS == "Darwin": subprocess.run(["osascript", "-e", 'tell application "System Events" to key code 145'])
    else: subprocess.run(["brightnessctl", "set", "10%-"])
...
def safe_close_app(target_app: str) -> str:
    """Close a named application by process name. Safer than blind Alt+F4."""
    if not target_app:
        return "Please specify which app to close (e.g. 'Discord', 'Notepad', 'Brave')."

    # Safety: never kill Lumina's own process
    _PROTECTED = {"python", "python3", "pythonw", "node", "server", "lumina"}
    t = target_app.lower().strip().replace(".exe", "")
    if t in _PROTECTED:
        return f"Refusing to close '{target_app}' — it may be part of Lumina."

    if _OS == "Windows":
        try:
            # Build a best-guess exe name
            exe_name = target_app.strip()
            ...
            # Fallback: try without .exe suffix
            result2 = subprocess.run(
                ["taskkill", "/F", "/IM", target_app.strip()],
                capture_output=True, text=True, timeout=8
            )
            return f"Closed {target_app}." if result2.returncode == 0 else f"Could not close '{target_app}': {result.stderr.strip()}"
        except Exception as e:
            return f"Failed to close '{target_app}': {e}"
    else:
        # macOS / Linux: use pkill
        try:
            subprocess.run(["pkill", "-f", target_app], timeout=8)
            return f"Closed {target_app}."
        except Exception as e:
            return f"Failed to close '{target_app}': {e}"
...
def open_file_explorer():
    ...
    elif _OS == "Darwin":  subprocess.Popen(["open", Path.home()])
    else:                  subprocess.Popen(["xdg-open", Path.home()])
...
# ── ACTION_MAP (shutdown/restart intentionally excluded for safety) ────────────

ACTION_MAP = {
    "volume_up": volume_up, "volume_down": volume_down,
    "mute": volume_mute, "unmute": volume_mute, "volume_mute": volume_mute,
    ...
    "louder": volume_up, "quieter": volume_down, "silence": volume_mute,
    ...
    "dim_screen": brightness_down, "brighten_screen": brightness_up,
    "sleep_display": sleep_display, "turn_off_screen": sleep_display,
    "screen_off": sleep_display, "display_off": sleep_display,
    "screen_sleep": sleep_display, "monitor_off": sleep_display,
    "turn_off_monitor": sleep_display,
    "pause_video": pause_video, "play_video": pause_video,
    "pause": pause_video, "play": pause_video, "toggle_play": pause_video,
    "stop_video": pause_video, "resume_video": pause_video,
    "close_app": close_app, "close_window": close_window,
    "quit_app": close_app, "exit_app": close_app, "kill_app": close_app,
    "full_screen": full_screen, "fullscreen": full_screen, "toggle_fullscreen": full_screen,
    ...
    "window_left": snap_left, "window_right": snap_right,
    "switch_window": switch_window, "alt_tab": switch_window, "next_window": switch_window,
    "show_desktop": show_desktop, "desktop": show_desktop, "hide_windows": show_desktop,
    "task_manager": open_task_manager, "open_task_manager": open_task_manager,
    "task_view": open_task_view,
    "screenshot": take_screenshot, "take_screenshot": take_screenshot, "capture_screen": take_screenshot,
    "lock_screen": lock_screen, "lock": lock_screen,
    "open_settings": open_system_settings, "system_settings": open_system_settings,
    "settings": open_system_settings, "preferences": open_system_settings,
    "file_explorer": open_file_explorer, "open_explorer": open_file_explorer,
    "explorer": open_file_explorer, "open_files": open_file_explorer,
    "run": open_run, "open_run": open_run,
    # shutdown/restart intentionally removed for safety
    "dark_mode": dark_mode, "toggle_dark_mode": dark_mode, "night_mode": dark_mode,
    "toggle_wifi": toggle_wifi, "wifi": toggle_wifi, "wifi_toggle": toggle_wifi,
    "focus_search": focus_search, "address_bar": focus_search, "url_bar": focus_search,
    "refresh_page": refresh_page, "reload_page": refresh_page, "reload": refresh_page, "refresh": refresh_page,
    ...
    "zoom_in": zoom_in, "zoom_out": zoom_out, "zoom_reset": zoom_reset, "reset_zoom": zoom_reset,
    "find_on_page": find_on_page, "search_page": find_on_page,
    "scroll_up": scroll_up, "scroll_down": scroll_down,
    "scroll_top": scroll_top, "scroll_bottom": scroll_bottom,
    "top_of_page": scroll_top, "bottom_of_page": scroll_bottom,
    "page_up": page_up, "page_down": page_down,
    "copy": copy, "paste": paste, "cut": cut, "undo": undo, "redo": redo,
    "select_all": select_all, "save": save_file, "save_file": save_file,
    "enter": press_enter, "press_enter": press_enter,
    "escape": press_escape, "press_escape": press_escape, "cancel": press_escape,
}


def _detect_action(description: str) -> dict:
    """Use Gemini to detect the intended action from a natural language description."""
    try:
        import json, re
...
def computer_settings(
    ...
    """
    Lumina computer settings & UI control action.

    parameters:
        action      : Action name (from ACTION_MAP). If omitted, description is used with Gemini.
        description : Natural language command (any language)
        value       : Optional value (volume level, text to type, repeat count, key name)
    """
    if not _PYAUTOGUI:
        return "pyautogui is not installed. Run: pip install pyautogui"

    params      = parameters or {}
    raw_action  = params.get("action", "") or ""
    ...
    if not raw_action and description:
        ...
        raw_action = (detected.get("action") or "").strip()
        if value is None:
            value = detected.get("value")

    action = (raw_action or "").lower().strip().replace(" ", "_").replace("-", "_")
    ...
    # Special-cased actions with value parameters
    ...
    # ── Safe close: route named-app closes through taskkill ──────────────────
    if action in ("close_app", "quit_app", "exit_app", "kill_app"):
        # If a specific target app is named (value or from description), use safe close
        target = str(value or "").strip() or description.strip()
        # Strip common phrases to get the bare app name
        import re as _re
        _app_match = _re.search(
            r"\b(discord|spotify|telegram|whatsapp|zoom|chrome|brave|firefox|edge|vlc|"
            r"notepad|calc|paint|word|excel|powerpnt|teams|slack|obs|vscode|code|"
            r"cmd|powershell|explorer|regedit|taskschd|winrar|7z)\b",
            target.lower() if target else ""
        )
        if _app_match:
            return safe_close_app(_app_match.group(1))
        elif not target or target.lower() in ("app", "window", "current", "this"):
            return (
                "⚠️ Please specify which app to close, e.g. 'close Discord' or 'quit Brave'. "
                "Without a target, I might close the wrong window."
            )
        else:
            return safe_close_app(target)

    # ── Recycle bin / OS system actions not handled here ─────────────────────
    _UNSUPPORTED_INTENTS = (
        "recycle_bin", "empty_recycle_bin", "clean_recycle_bin",
        "clear_recycle_bin", "delete_recycle_bin", "empty_trash",
        "disk_cleanup", "defrag", "format_drive",
    )
    if action in _UNSUPPORTED_INTENTS:
        return (
            f"'{action.replace('_', ' ').title()}' is a system-level operation not "
            "supported through keyboard automation. Use cmd_control or file_controller instead."
        )
